...
public interface IDemandeModifDao extends JpaRepository<DemandeModif, Integer> {
    ...
    @Query("select d from DemandeModif d where d.etatDemande.numEtat = 10 and d.dateModification < :dateLimite order by d.dateModification asc")
    List<DemandeModif> getNextDemandeToDeleteBefore(@Param("dateLimite") Date dateLimite);
