...
#  then
#   java -XX:MaxRAMPercentage=80 org.springframework.boot.loader.launch.JarLauncher --spring.batch.job.name=traiterLigneFichierExemp --bigVolume=true --server.port=0 &
#  fi
#  if [[ $isEnAttenteSmallVolume == 'true' && $(pgrep -cf "traiterLigneFichierExemp --bigVolume=false") -lt 1 ]];
#  then
#   java -XX:MaxRAMPercentage=80 org.springframework.boot.loader.launch.JarLauncher --spring.batch.job.name=traiterLigneFichierExemp --bigVolume=false --server.port=0 &
#  fi
  ...
  then
    java -XX:MaxRAMPercentage=80 org.springframework.boot.loader.launch.JarLauncher --spring.batch.job.name=traiterLigneFichierExemp --server.port=0 &
  fi
fi
